...
def main():
    ...
    # 25088 = 2 * 112 * 112 . 2 bytes for each pixels, I need 896 = 2 * 112 * 4 --> 2*512
    unit_size = 1024 * n_video

    # For parameter data tacked on to each frame (previously 512), right now I only need 2 byte for eyelid location, but let's say 4 byte
    # unit_size += 4
    ...
    if sec_mask_filename != None:
        ...
        sec_mask = np.zeros((4,112)).astype('int64')

    if pri_mask_filename != None:
        pri_mask = load_mask(args.pri_mask)
    else:
        pri_mask = np.zeros((4,112)).astype('int64')
    ...
    if args.reuse_raw:
        ...
        try:
            # "rb" open file for reading only in a binary format
        ...
        if (num_frames != None):
            ...
            while True:
                data_a = ''
                data_b = ''
                # What are these values? # 7168 = 2 * 32 * 112 # 3584 = 2 * 16 * 112
                if n_video != 0:
                    # 7168 = 2 * 32 * 112
                    data_a += input_file.read(896)
                ...
                data_param = input_file.read(128)
...
def is_eof(result_data):
    values = struct.unpack('b' * 128, result_data)
...
def read_images(image_file, mask_data, out_filename, num_frames, columnwise, outdoor_mask=None):
    ...
    images = np.zeros((num_frames, 4 * 112))
...
def read_packed_image(image_file, mask_data, out_filename, columnwise, index):
    image = np.zeros((4,112))
    for i, j in itertools.product(range(4), range(112)):
...
def save_frames(filename, data, save_options, all_param_fields):
    ...
    for data_pair in data.items():
        ...
        for (i, image_orig), param_fields in itertools.izip(enumerate(images), all_param_fields):
            image = image_orig.reshape((4,112),order='F')
