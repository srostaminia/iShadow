...
model_sizes = [[2, 3], [2, 4], [2, 5], [3, 4], [2, 7], [4, 4], [3, 6], [4, 5], [2, 11], [4, 6]]
...
for name in names:
	for ftype in types:
		for size in model_sizes:
			...
			for datum in data:
				datum_pathname = 'hrange/' + name + '/' + size_text + '/' + ftype + '/' + datum + '/'

				if not os.path.exists(datum_pathname):
					os.makedirs(datum_pathname)

			size_pathname = 'hrange/' + name + '/' + size_text + '/' + ftype + '/'
