function [f,g] = logisticmlp_obj(w,X,y,nHidden)
...
if(isempty(nHidden)) %Have no hidden layer - just do tanh regression
  ...
  %Compute error matrix and objective
  relativeErr = yhat-y;
  f = sum(relativeErr(:).^2);

  %Compute gradient
  if nargout > 1
    g = X'*(2*relativeErr.*sech(ip).*2);
    g = g(:);
  end

else %Have a real hidden layer
  ...
  if nargout > 1
      ...
      elseif length(nHidden)==0
        gOutput =  X'*err;
      end
  end
      
  % Put Gradient into vector
  if nargout > 1 & ~isempty(nHidden)
      g = zeros(size(w));
      if(~isempty(nHidden))
      ...
      g(offset+1:offset+nHidden(end)*nTargets) = gOutput(:);

  elseif nargout > 1 & isempty(nHidden)
      g = gOutput(:);
  end
end
