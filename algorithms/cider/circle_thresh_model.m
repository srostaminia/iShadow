function [c,r]=circle_thresh_model(X,xy_target,chord_length,~)
    c = [0, 0];
    r = 0;
    ...
    % Calculate threshold from "landing point" (previous prediction)
    landp = X(rc_target(1) - 1:rc_target(1) + 1, rc_target(2) - 1:rc_target(2) + 1);
    thresh = mean(landp(:)) * 0.9;
%     thresh = X(rc_target(1), rc_target(2)) * 0.7;
    ...
    for i=1:2
        ...
        % This try/catch is a hideous hack and I am ashamed to have written it. - Addison
        try
            % First try using the first two pixels in a row that fall above the threshold   
            if (i==1)
                ...
                % If that doesn't give a good fit, try looking for the first single pixel that falls above the threshold
