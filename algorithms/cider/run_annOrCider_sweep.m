...
if strcmp(modelName,'idealLine')
    fprintf('\nIDEAL LINE Case: previous gout for line\n');
    
elseif strcmp(modelName,'cider') %(input gout as the previous prediction)
    fprintf('\nRealistic LINE Case: Use ANN previous prediction\n');
...
if strcmp(testIndOn,'randomCross')
    ...
    for a=1:5
        ...
        rep_data=load(rep_files{a},'these_results');
        cd(origin);
        test_ind=rep_data.these_results.test_ind;
        XCell{a}=X(test_ind,:);
        goutCell{a}=gout(test_ind,:);
        
        if nDim==3
            sqrtRadEllipseCell{a}=sqrtRadEllipse(test_ind,:);
            %trueAreaCell{a}=trueArea(test_ind,:);
        end
    end
    % elseif strcmp(testIndOn,'8020')
    %     nFrames=size(X,1);
    %     ind_start=nFrames-(ceil(0.8*nFrames));
    %     X=X(ind_start:end,:);
    %     gout=gout(ind_start:end,:);
    %     sqrtRadEllipse=sqrtRadEllipse(ind_start:end,:);
    elseif strcmp(testIndOn,'100')
        fprintf('\nWarning: using all data as test data');

else
    fprintf('\nERROR: Wrong text. Input testIndOn again.');
    return;
end
...
for i=1:length(lambda_folders)-1
    ...
    for j=1:length(rep_files)-1
        ...
        if strcmp(modelName,'cider') || strcmp(modelName,'idealLine')
            ...
            %subsampled pixels
            perc_LineUsed=100*(1-sum(ann_used | ~filter)/totalFrame);
        ...
        if nDim==3
            ...
            elseif strcmp(thirdVarName,'area')
                ...
                fprintf('ERROR: fix the thirdVar string.');
            end
            ...
            if strcmp(modelName,'cider') || strcmp(modelName,'idealLine')
                ...
                save('-V7',sprintf('cider_%srep%d.mat',sub_model,j),...
                    'ind','chord_length','center','radii','ann_used','filter','center','mean_model',...
            ...
            elseif strcmp(modelName,'ann')
                ...
                save('-V7',sprintf('ann_%srep%d.mat',sub_model,j),...
                    'ind','center','radii','filter','center','mean_model',...
                    'indActivePercent');
                % save('-V7',sprintf('ann_%srep%d.mat',sub_model,j),...
