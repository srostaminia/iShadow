        ...
        for u=1%:length(uniList)
            ...
            for lam=1:length(lambdas)
                ...
                for i=1:5
                    ...
                    disp(sprintf('%s - %s - %s - %s%i', name, uniList{u}, lambdas{lam}, 'rep',i));
                  %save v7 file
                    if matlabOn==0
                    ...
                    cd(path_sh);
                        ...
                        %if exist([data_title,'.sh'])==2

                            if matlabOn==0

			      command=['qsub', ' -cwd', ' -o', ' ',strcat(data_title,'_stdout.txt'), ' -e', ' ',strcat(data_title,'_stderr.txt'),' ', data_title,'.sh'];
disp(command)
