function results = test_model(W_groupSparse,Xtrain,ytrain,Xtest,ytest,params)
      ...
      for i=1:nHidden-1
        ...
        imagesc(reshape(Wih(1:end-1,i+1),[56,56]));colormap(covcolor); caxis([-cm,cm]);colorbar;
      end
      ...
      dist_test = sqrt(sum((56*(ytest - yhat_test)).^2,2));
      dist_train = sqrt(sum((56*(ytrain - yhat_train)).^2,2));
      ...
      %  imagesc(reshape(Xtrain(ind(i),2:end),[56,56]));colormap(gray); caxis([0, 1]);
      %  input('next>>');
      %end
