...
% Scale image down to 56x56 resolution
[X_samples, Y_samples] = meshgrid(linspace(1,112,56), linspace(1,111,56));

X_resize=zeros(size(X,1),56*56);

for n=1:size(X,1)
  X_resize(n,:) = reshape(interp2(reshape(X(n,:), [111, 112]), X_samples, Y_samples), [1, 56 * 56]);
end
...
g          = bsxfun(@times,gout,1./[56,56]); %Normalize gaze matrix
  ...
  for l=1:length(params(c).lambdas) 
    ...
    if(~exist(fname) | from_scratch);
      ...
      switch(params(c).subset)
        ...
        case 'rand'
          K = max(1,round(56*56*(1-lambda)));
          ind = rect_grid_randsample([56 56],K);
          if(~isempty(Winit))
        ...
        case 'kmed'
          K = max(1,round(56*56*(1-lambda)));
          ind = rect_grid_kmedioids([56 56],K);
          if(~isempty(Winit0))
            Winitsub = params_mask(Winit0,nVars,params(c).nHidden,ind);
          else
            Winitsub = Winit0;
      ...
      these_results= test_model_resize(W_groupSparse,Xtrain_rescale,ytrain,Xtest_rescale,ytest,params(c));
      ...
      save_model_resize(W_groupSparse,params,fname)
    ...
    pix = (1-results(l,r).sparsity)*56*112;
    ...
    for i=1:params(c).nHidden-1
      axes(ha(i));
      imagesc(reshape(Wih(1:end-1,i+1),[56,56]));colormap(covcolor); caxis([-cm,cm]);
      set(gca,'xtick',[],'ytick',[]) 
    end
    colormap(covcolor);
    caxis([-cm,cm]);
    ...
    maskimg = double(reshape(mask(1:end-1),[56,56]));
    ...
    %   hold off;
    %   eye_img = reshape(X(n,1:end-1),[56,56]);
    %   out_img = reshape(double(Out(n,1:end))/1000,[111,112]);
    %   imagesc([fliplr(eye_img), out_img]);
    %   hold on;
    %   plot(56+56*pred(1),56*pred(2),'r+','markersize',20,'linewidth',3);
    %   plot(56+56*g(n,1),56*g(n,2),'bo','markersize',20,'linewidth',3);
    %   axis off;
    %   colormap(gray);
    %   drawnow;
