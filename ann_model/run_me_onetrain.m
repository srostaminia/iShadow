...
params(1).hold_out_idx = load('onetrain.txt')
...
[X,Out,gout,hold_out_size] = join_eye_data(names, params(1).hold_out_idx);
fprintf('Done.\n');
...
for c=[1]
  ...
  Ntrain = hold_out_size;
  ind = 1:N;
  Xtrain= X(ind(1:Ntrain),:);
  Xtest = X(ind(Ntrain+1:end),:);

  ytrain= g(ind(1:Ntrain),:);
  ytest = g(ind(Ntrain+1:end),:);
