...
function [errStEachSub,meanErrAllSub,stdErrAllSub,indPerForAllSub,meanErr_line_ForAllSub,stdErr_line_ForAllSub,meanAnnUsedForAllSub,stdAnnUsedForAllSub,meanLineUsedForAllSub,stdLineUsedForAllSub]=graph_error_model_current(indFile,linestyle,modelName,subList,rootDir,lightingSubFolder,objToPlot)
...
for i=2%:length(subList)
    ...
    indFile;
    subName=subList{1};
    subNameFolder=subNameFolderCell{indFile}
    ...
    for k=1:length(lambdaFolderList)
        ...
        for m=1:nRep
            ...
            dir= fullfile(rootDir,strcat(subNameFolder),uni,'results',lambdaFolder,strcat(modelName,'_rep',num2str(m),'.mat'));
            ...
            elseif strcmp(objToPlot,'pupilLoc_pixel')
                ...
                if strcmp(modelName,'cider')
                    ...
                    err_line_MatEachSub(m,k,j)=data.mean_line.center.diff;
                    annUsedMatEachSub(m,k,j)=100*sum(data.ann_used)/length((data.ann_used));
                    
                    
                elseif strcmp(modelName,'ann')
                    data.mean_ann.center.diff
    ...
    if strcmp(modelName,'cider')
        ...
        for i=1:nUsers %nUsers x nLambda: 6x10
            ...
            %stdAnnUsedForAllSub(i,:)=annUsedStEachLambdaEachSub.std.(uni).(subName);
        end
