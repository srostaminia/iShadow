...
for i=1:length(subList)
    ...
    %subNameFolder=subNameFolderCell{i}
        ...
        for m=1:nRep
            ...
            elseif strcmp(objToPlot,'pupilLoc_pixel')
                ...
                if strcmp(modelName,'cider')
                    ...
                    errMatEachSub(m,k,j)=data.err;%mean_cider.center.diff;%mean(data.perAreaDiff);%100*data.diffArea./(pi*data.sqrtRadEllipse(( logical(sum(data.pred,2))),:).^2);
                    err_line_MatEachSub(m,k,j)=data.err_Line;%mean_line.center.diff;
                    annUsedMatEachSub(m,k,j)=100*sum(data.ann_used)/length((data.ann_used));
                ...
                elseif strcmp(modelName,'ann')
                    errMatEachSub(m,k,j)=data.err;%mean_ann.center.diff;%mean(data.perAreaDiff);%100*data.diffArea./(pi*data.sqrtRadEllipse(( logical(sum(data.pred,2))),:).^2);
                end
