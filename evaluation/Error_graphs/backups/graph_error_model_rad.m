...
function [errStEachSub,uniFolderList,stdErrAllSub,meanErr_line_ForAllSub,stdErr_line_ForAllSub]=graph_error_model_rad(lighting,modelName,subList,subDir,objForTitle,ylabelunit,dataObj,title,fileToOpen,fileToSave,legendToPut)
...
%function [errStEachSub,meanErrAllSub,stdErrAllSub,meanErr_line_ForAllSub,stdErr_line_ForAllSub]=graph_error_model_rad(lighting,modelName,subList,rep_dir,dataObj)
    ...
    for j=1:length(uniFolderList)
        ...
        for k=1:nLambda
            ...
            for m=1:nRep
                ...
                repdir= fullfile(subDir,subFolderName,uni,'results',lambdaFolder,sprintf(fileToOpen,m));
                
                repdir
                %repdir= fullfile(rootDir,strcat('awesomeness_irb_',object),strcat(subName,'_',lighting),uni,'results',lambdaFolder,strcat(modelName,'_rep',num2str(m),'.mat'));
...
hYLabel=ylabel(ylabelunit);
hXLabel=xlabel('ANN size (incrementing)');
%title(strcat('subject: ',subName));
hLegend=legend(strcat(subList,'-',legendToPut));

if length(title)>0
    hTitle=suptitle(title);
else
    hTitle=suptitle({strcat(modelName,': Error of predicted ',objForTitle,' for 6 different users for different ANN sizes'),'Each of the ten points represents the average of errors for 5 repetitions of the experiment.','ANN size increases from left to right.'});
end

format_graph_paperQ(hTitle,hXLabel, hYLabel,hLegend,fileToSave)
